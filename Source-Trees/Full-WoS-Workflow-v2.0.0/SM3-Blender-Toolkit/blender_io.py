...
def _parse_skeleton_any(filepath):
    """Parse raw .skel or resolve a .wrap.skel through the proven v0.4.0 path.

    This is intentionally restored verbatim in behavior from the old working
    toolkit so Rename Vertex Groups always sees the real game skeleton rather
    than the outer WRAP container.
    """
    from .wrap_io import is_wrap_file, unwrap_file
    if not is_wrap_file(filepath):
        return parse_skeleton(filepath)
    flat, _info = unwrap_file(filepath)
    suffix = ".skel"
    fd, tmp = tempfile.mkstemp(prefix="sm3_wrap_skel_", suffix=suffix)
    os.close(fd)
    try:
        Path(tmp).write_bytes(flat)
        return parse_skeleton(tmp)
    finally:
        try:
            os.remove(tmp)
        except OSError:
...
def import_skeleton(
    ...
    *,
    ...
    skel = _parse_skeleton_any(filepath)
...
def safe_export_skeleton(arm_obj: bpy.types.Object, output_path: str) -> str:
    ...
    source_skel = _parse_skeleton_any(source)
    ...
    *,
    ...
    if skel_path:
        ...
        if import_skeleton_if_found:
            ...
            skel = _parse_skeleton_any(skel_path)
    ...
    if not safe_viewer_mode:
        collection["sm3_toolkit_version"] = "1.2.0-spider-baked-atlas-prep"
...
def _skeleton_index_name_map(armature_obj: bpy.types.Object) -> Dict[int, str]:
    ...
    if source and os.path.isfile(source):
        skel = _parse_skeleton_any(source)
