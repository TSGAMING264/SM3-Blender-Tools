...
def register_all(addon_info=None, *args, **kwargs) -> None:
    """Register Material Combiner components without the legacy self-updater.

    ``*args``/``**kwargs`` are accepted deliberately so stale/alternate wrapper
    calls cannot break addon registration on modern Blender.
    """
    _register_classes()
...
def unregister_all(*args, **kwargs) -> None:
    """Unregister Material Combiner components without updater teardown."""
    _unregister_classes()
    unload_smc_icons()
