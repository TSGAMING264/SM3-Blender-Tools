...
from ...sm3_scope import resolve_scope
...
class MaterialListRefreshOperator(bpy.types.Operator):
    ...
    def execute(self, context: bpy.types.Context) -> Set[str]:
        ...
        scoped, scope_mode = resolve_scope(context)
        if scoped is not None:
            scene["sm3_combiner_active_build"] = "v1.8.1"
            self.report({"INFO"}, f"SM3 v1.8.1 WORK COPY ONLY: {scoped.name}")
        return {"FINISHED"}
    ...
    def _get_eligible_objects(
        ...
        # SM3/WoS hard scope v1.7.9.  Do NOT rely only on the companion
        # toolkit's scene string: detect protected work copies directly from
        # their object metadata on every refresh.  This includes the internal
        # refresh performed by Create Atlas.
        scoped, scope_mode = resolve_scope(context)
        if scoped is not None:
            return {scoped}

        # If an SM3 work copy exists but resolution is ambiguous, fail closed.
        # Falling back to all visible meshes here is exactly the bug that could
        # pull donor/source meshes into the atlas.
        if scope_mode == "AMBIGUOUS":
            return set()

        # Generic/non-SM3 Material Combiner behavior remains unchanged.
        return {obj for obj in context.visible_objects if eligible(obj)}
