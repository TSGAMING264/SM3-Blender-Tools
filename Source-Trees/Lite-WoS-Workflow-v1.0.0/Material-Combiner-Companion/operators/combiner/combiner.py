...
class Combiner(bpy.types.Operator):
    ...
    def invoke(
        ...
        # v1.7.9 hard guard: if this .blend contains an SM3 protected work
        # copy, Create Atlas may operate on that one object only.  This check
        # is independent of UI state and catches stale lists/version collisions
        # before any destructive UV/material changes occur.
        scoped, scope_mode = resolve_scope(context)
        if scope_mode == "AMBIGUOUS":
            return self._return_with_message(
                "ERROR",
                "Multiple SM3 combiner work copies found - select the intended work copy and refresh",
            )
        if scoped is not None:
            listed_objects = {
                item.ob
                for item in scn.smc_ob_data
                if item.type == globs.CombineListTypes.OBJECT and item.ob is not None
            }
            if listed_objects != {scoped}:
                return self._return_with_message(
                    "ERROR",
                    "SM3 protected scope mismatch - atlas blocked before modifying the model",
                )
