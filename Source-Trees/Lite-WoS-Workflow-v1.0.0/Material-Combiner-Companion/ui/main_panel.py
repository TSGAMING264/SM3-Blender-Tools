...
from ..sm3_scope import scope_status
...
class MaterialCombinerPanel(bpy.types.Panel):
    ...
    def _create_materials_list(
        ...
        build_box = list_column.box()
        build_box.label(text="SM3 HARD-SCOPE BUILD v1.8.1", icon="LOCKED")
        scoped, scope_mode = scope_status(context)
        if scoped is not None:
            scope_box = list_column.box()
            scope_box.label(text="WORK COPY ONLY — other meshes are excluded", icon="LOCKED")
            if scope_mode != "SCENE":
                scope_box.label(text=f"Auto-detected ({scope_mode.lower()})")
            scope_box.label(text=scoped.name)
        elif scope_mode == "AMBIGUOUS":
            scope_box = list_column.box()
            scope_box.alert = True
            scope_box.label(text="SM3 scope blocked: multiple work copies", icon="ERROR")
            scope_box.label(text="Select the intended __SM3_COMBINER_WORK and refresh")
