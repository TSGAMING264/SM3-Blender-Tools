...
class SMC_UL_Combine_List(bpy.types.UIList):
    ...
    def filter_items(
        ...
        # v1.8.0: when SM3 protected scope is active, stale list rows from
        # older/alternate combiner installations are hidden in the UI too.
        # This is visual defense-in-depth; refresh/create still hard-scope the
        # actual operation to the protected work copy.
        scoped, _scope_mode = resolve_scope(context)
        if scoped is not None:
            for idx, item in enumerate(collection):
                ob = getattr(item, "ob", None)
                if ob is not None and ob != scoped:
                    filter_flags[idx] = 0
        ...
        # Re-apply hard scope after text filtering so user filters can never
        # re-show stale non-work rows.
        if scoped is not None:
            for idx, item in enumerate(collection):
                ob = getattr(item, "ob", None)
                if ob is not None and ob != scoped:
